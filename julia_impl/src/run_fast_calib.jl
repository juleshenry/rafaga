...
# Using an initial guess close to what we got before to speed it up
res_mrlrj = optimize(obj_MRLRJ, [2.18, 0.60, 2.31, 2.48, 3.66], NelderMead(), Optim.Options(iterations=300))
pe_mrlrj = Optim.minimum(res_mrlrj)

println("--- Calibration Results (Percentage Error) ---")
@printf("MRLR Baseline Model PE: %.2f%%\n", pe_mrlr * 100)
@printf("MRLRJ Jump Model PE: %.2f%% (Implied Accuracy: %.2f%%)\n", pe_mrlrj * 100, 100 - pe_mrlrj * 100)
